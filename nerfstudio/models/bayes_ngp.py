...
from nerfstudio.fields.bayes_ngp_field import TCNNBayesNGPField
...
@dataclass
class BayesNGPModelConfig(ModelConfig):
    """Bayes NGP Model Config"""

    _target: Type = field(
        default_factory=lambda: BayesNGPModel
    )  # We can't write `BayesNeRFModel` directly, because `BayesNeRFModel` doesn't exist yet
...
class BayesNGPModel(Model):
    """Bayes NGP model

    Args:
        config: Bayes NGP configuration to instantiate model
    """

    config: BayesNGPModelConfig
    field: TCNNBayesNGPField

    def __init__(self, config: BayesNGPModelConfig, **kwargs) -> None:
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        # output density
        density = field_outputs[FieldHeadNames.DENSITY].squeeze(-1)
        density_sum = self.renderer_accumulation(weights=density, ray_indices=ray_indices, num_rays=num_rays)
        outputs["density_sum"] = density_sum

        k_empty = 10
        weights_log_sum = self.renderer_accumulation(weights=torch.log(1 + weights * k_empty), ray_indices=ray_indices, num_rays=num_rays)
        outputs['weights_log_sum'] = weights_log_sum
        ...
        # # [debug] check outputs shapes
        # print('num_rays: ', num_rays)
        # for key, output in outputs.items():
        #     print('key: {}, shape: {}'.format(key, output.shape))
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None):
        ...
        loss_dict["rgb_loss"] = ((image[mask] - outputs["rgb"][mask]) ** 2 / (betas[mask] ** 2)).mean()
        
        # sparity loss (choice 1: nerf-w, mean of densities)
        # loss_dict["density_loss"] = 0.01 * (outputs["density_sum"].squeeze(-1) / outputs["num_samples_per_ray"]).mean()

        # # sparsity loss (choice 2: sjc, mean of log weights)
        # loss_dict["empty_loss"] = 1e-4 * (outputs["weights_log_sum"].squeeze(-1) / outputs["num_samples_per_ray"]).mean()
