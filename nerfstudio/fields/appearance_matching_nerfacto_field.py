...
class TCNNNerfactoField(Field):
    ...
    Args:
        ...
        test_appearance_embedding: what appearance embedding to use at test time
    ...
    def __init__(
        ...
        self.test_appearance_embedding = test_appearance_embedding
    ...
    def get_outputs(self, ray_samples: RaySamples, density_embedding: Optional[TensorType] = None):
        ...
        if self.training:
            ...
            if self.test_appearance_embedding == 'average':
                embedded_appearance = torch.ones(
                    (*directions.shape[:-1], self.appearance_embedding_dim), device=directions.device
                ) * self.embedding_appearance.mean(dim=0)
            elif self.test_appearance_embedding == 'zero':
                embedded_appearance = torch.zeros(
                    (*directions.shape[:-1], self.appearance_embedding_dim), device=directions.device
                )
            elif self.test_appearance_embedding == 'cam_idx':
                embedded_appearance = self.embedding_appearance(camera_indices)
            else:
                raise NotImplementedError('Test-time appearance method {} not implemented'.format(self.test_appearance_embedding))
