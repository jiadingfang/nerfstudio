...
class TCNNBayesNeRFField(Field):
    """Compound Bayes NeRF Field that uses TCNN
    ...
    def __init__(
        ...
        # # predict uncertainty from density embedding and position encoding (choice 1)
        ...
        # predict uncertainty from density embedding detached (choice 2)
        self.mlp_pred_uncertainty = tcnn.Network(
            n_input_dims=self.geo_feat_dim,
            n_output_dims=hidden_dim_transient,
            network_config={
    ...
    def get_outputs(self, ray_samples: RaySamples, density_embedding: Optional[TensorType] = None):
        ...
        # positions = ray_samples.frustums.get_positions()
        # positions_flat = self.position_encoding(positions.view(-1, 3))
        # pred_uncertainty_inp = torch.cat([positions_flat, density_embedding.view(-1, self.geo_feat_dim)], dim=-1)
...
field_implementation_to_class = {"tcnn": TCNNBayesNeRFField, "torch": TorchNerfactoField}
