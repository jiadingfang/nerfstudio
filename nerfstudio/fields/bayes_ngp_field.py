...
"""
Bayes NGP field implementations using tiny-cuda-nn, torch, ....
"""
...
from nerfstudio.field_components.field_heads import FieldHeadNames, UncertaintyFieldHead
...
class TCNNBayesNGPField(Field):
    """TCNN implementation of the Bayes NeRF field.
    ...
    def __init__(
        ...
        # # predict uncertainty from density embedding directly with uncertainty head (choice 1)
        # self.field_head_uncertainty = UncertaintyFieldHead(in_dim=self.geo_feat_dim)

        # predict uncertainty from density embedding detached with 2-layer NN (choice 2)
        self.mlp_pred_uncertainty = tcnn.Network(
        ...
        self.field_head_uncertainty = UncertaintyFieldHead(in_dim=self.mlp_pred_uncertainty.n_output_dims)
    ...
    def get_outputs(self, ray_samples: RaySamples, density_embedding: Optional[TensorType] = None):
        ...
        # # Output Uncertainty (choice 1)
        # uncertainty = self.field_head_uncertainty(density_embedding)
        # outputs[FieldHeadNames.UNCERTAINTY] = uncertainty

        # # Output Uncertainty (choice 2)
        # density_embedding_copy = density_embedding.clone().detach()
        # pred_uncertainty_inp = torch.cat(
        #     [
        #         density_embedding_copy.view(-1, self.geo_feat_dim),
        #     ],
        #     dim=-1,
        # )
        # x = self.mlp_pred_uncertainty(pred_uncertainty_inp).view(*outputs_shape, -1).to(directions)
        # outputs[FieldHeadNames.UNCERTAINTY] = self.field_head_uncertainty(x)

        # Output Uncertainty (choice 3)
        pred_uncertainty_inp = torch.cat(
            [
